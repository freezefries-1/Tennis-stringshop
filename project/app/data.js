// SportCraft Workshop — seed data for the Phase 1 shell. Replaced by Postgres queries in later phases.
(function () {
  ...
  const fmt = (n, dp) => "$" + n.toLocaleString("en-SG", { minimumFractionDigits: dp == null ? 2 : dp, maximumFractionDigits: dp == null ? 2 : dp });
  const fmt0 = (n) => "$" + Math.round(n).toLocaleString("en-SG");

  const sep = new Date("2026-09-22");
  const month = { rev: 5240, cogs: 1780, exp: 1180, jobs: 89 };
  const ytd = { rev: 49_440, cogs: 16_780, exp: 10_940 };
  ...
  window.SC_DATA = {
    ...
    month: Object.assign({}, month, { gross: month.rev - month.cogs, net: month.rev - month.cogs - month.exp, avgJob: month.rev / month.jobs }),
    ytd: Object.assign({}, ytd, { gross: ytd.rev - ytd.cogs, net: ytd.rev - ytd.cogs - ytd.exp }),
    unbilled: 2,
