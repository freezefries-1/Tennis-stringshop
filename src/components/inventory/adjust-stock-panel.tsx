...
import { recordManualAdjustmentAction, updateBatchCostAction } from "@/app/inventory/actions";
...
export function AdjustStockPanel({ productId, batches, unit }: { productId: string; batches: StringInventoryBatch[]; unit: StockUnit }) {
  ...
  async function submit() {
    ...
    if (open === "edit_cost") {
      ...
      setSaving(true);
      setError(null);
      await updateBatchCostAction({ batchId, purchaseCostCents: cents, reason }, productId);
      setSaving(false);
      setOpen(null);
      router.refresh();
      return;
